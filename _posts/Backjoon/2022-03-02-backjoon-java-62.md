...
`getArr`이라는 메서드를 만들어서 ```java a[i + 1] - a[i]```이라는 규칙을 적용시켰다. 여기서 중요한 점은 k번 만큼 규칙을 적용시키는 것이다. 처음에 입력받은 숫자들을 넣은 배열로 초기화 시킨 배열과, 반복할 횟수를 넣는다. 그리고 거기서 리턴되는 값을 다시 매개변수 배열로 대입시켜서 k번 돌아가게 하였다. 