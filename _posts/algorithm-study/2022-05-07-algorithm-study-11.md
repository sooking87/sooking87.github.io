...
말 그대로 정렬 속도가 매우 빠르다. 진행방식은 병합 정렬과 비슷한 **_분할 & 정복_** 방식이다. 병합 정렬의 경우는 리스트를 **_절반_** 으로 나누어 분할 정복을 진행하지만 퀵 정렬의 경우 피벗 기준으로 나눈 **_크기가 다를 수 있는 두 배열_** 을 각각 다시 분할하여 크기가 1인 리스트가 될 떄까지 분할 후 정복하는 방식이다.
